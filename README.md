...
| [`easeofuse/course_management_subtle_actions.scss`](easeofuse/course_management_subtle_actions.scss) | easeofuse | global | Dims the per-row controls on the course and category management page — drag handle, bulk checkbox and action icons — and brings them back as soon as you point at, or tab into, the row. The selected category and course keep theirs at full strength. |
...
| [`easeofuse/course_management_bulk_editing_only.scss`](easeofuse/course_management_bulk_editing_only.scss) | easeofuse | global | Shows the bulk and sorting tools of the course management page — sorting categories and courses, the per page selector and the two bulk move forms — only while edit mode is switched on. Creating a category or course and the per row actions stay available. |
